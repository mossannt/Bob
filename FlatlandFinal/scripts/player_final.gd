...
func _physics_process(delta: float) -> void:
	...
	var input_vector: Vector2 = keyboard if keyboard.length() > 0.05 else move_input
	...
	velocity.y = -0.1 if is_on_floor() else velocity.y - gravity * delta
	move_and_slide()
...
func interact() -> void:
	var ray: RayCast3D = $Head/Camera3D/InteractRay
	if ray.is_colliding():
...
func _unhandled_input(event: InputEvent) -> void:
	...
	if event is InputEventScreenTouch:
		...
		if touch.pressed and touch.position.x > get_viewport().get_visible_rect().size.x * 0.42:
